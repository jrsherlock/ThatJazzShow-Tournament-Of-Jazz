  ...
  if (!timeLeft) {
    return (
      <div className={`text-center ${className}`}>
        <p className="text-[#D4A843] font-[family-name:var(--font-playfair)] text-lg italic">
          Submissions Closed
        </p>
      </div>
    );
      ...
      {units.map((unit, i) => (
        <div key={unit.label} className="flex items-center gap-3 sm:gap-5">
          <div className="flex flex-col items-center">
            <span className="text-[#D4A843] font-[family-name:var(--font-playfair)] text-3xl sm:text-5xl font-bold tabular-nums leading-none">
          ...
          {i < units.length - 1 && (
            <span className="text-[#D4A843]/40 text-2xl sm:text-4xl font-light -mt-4">:</span>
          )}
        </div>
      ))}
    </div>
